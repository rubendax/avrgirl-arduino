...
Connection.prototype._pollForOpen = function (callback) {
  ...
  var poll = awty(function (next) {
    _this.serialPort.open(function (error) {
      ...
      if (error) {
        console.log("Re-initializing Serial Port connection");
        Serialport.list().then(ports => {
          for (var curPort of ports) {                               // Iterate through all available ports.
            console.log("Trying to match PID: " + curPort.productId);
            for (var curPID of _this.board.productId) {              // Iterate through all possible PIDs of the target board.
              console.log("with: " + curPID.slice(2, 6));            // Transform 6-character hex string into 4 character hex string.
              if (curPort.productId == curPID.slice(2, 6)) {         // Does PID at the current port match the PID of the target board?
                console.log("Found device: " + _this.board.name + "\n at port: " + curPort.path + "\n which currently has productID: " + curPort.productId);
                _this.options.port = curPort.path;                   // Use the current port.
                break;
              }
            }
          }
          console.log("Trying to open port: " + _this.options.port);
